...
export function ThemeSwitcher() {
  ...
  React.useEffect(() => {
    ...
    const normalized = savedTheme === 'enver' ? 'ember' : savedTheme === 'space' ? 'night' : savedTheme;
  ...
  return (
    <div className="flex items-center pl-3 border-l h-5 border-border">
      <DropdownMenu>
        ...
        <DropdownMenuContent align="end" className="w-[140px] rounded-xl p-1 bg-popover border border-border">
          {themes.map((theme) => (
            <DropdownMenuItem
              key={theme.id}
